...
# ---------------------------------------------------------------------------
ENDPOINT_URL = "https://script.google.com/macros/s/AKfycbwChrOsdIy3KVr8NmRR7kGwgrfPIO6GicmSicEtxsz6a9ugLn0ghze-i9Fsm0Ux6KWAPQ/exec"
UPLOAD_TOKEN = "sO9iclopqjTYnOiWrZfD4Y59BrSbabaY33kDrKhGKlc"
DEFAULT_DAYS = 365
...
class CollectorApp:
    def __init__(self, root):
        ...
        tk.Label(form, text="Days of history:").grid(row=2, column=0, sticky="e", **pad)
        self.days = tk.Entry(form, width=28)
        self.days.insert(0, str(DEFAULT_DAYS))
        self.days.grid(row=2, column=1, **pad)

        # --- Metadata (written into the CSV; needed for the research) ---
        tk.Label(form, text="Age range:").grid(row=3, column=0, sticky="e", **pad)
        self.age = tk.StringVar(value=AGE_OPTIONS[0])
        tk.OptionMenu(form, self.age, *AGE_OPTIONS).grid(row=3, column=1, sticky="ew", **pad)

        tk.Label(form, text="Contraception:").grid(row=4, column=0, sticky="e", **pad)
        self.contraception = tk.StringVar(value=CONTRACEPTION_OPTIONS[0])
        tk.OptionMenu(form, self.contraception, *CONTRACEPTION_OPTIONS).grid(row=4, column=1, sticky="ew", **pad)

        tk.Label(form, text="Cycle regularity:").grid(row=5, column=0, sticky="e", **pad)
        self.regularity = tk.StringVar(value=REGULARITY_OPTIONS[0])
        tk.OptionMenu(form, self.regularity, *REGULARITY_OPTIONS).grid(row=5, column=1, sticky="ew", **pad)

        tk.Label(form, text="Garmin device model:").grid(row=6, column=0, sticky="e", **pad)
        self.device = tk.Entry(form, width=28)
        self.device.grid(row=6, column=1, **pad)
    ...
    def start(self):
        ...
        try:
            ...
            messagebox.showwarning("Invalid", "Days of history must be a positive number.")
            return
